...
The limit first comes into effect at the moment when the game advances to the next text box, and each line of the text currently in the text box's text buffer has to be copied to the text buffer inside a new text log line entry being created. These text buffer fields inside text log line entries are fixed at the size of 96 bytes – a maximum of 48 full-width characters can fit into one, and since in the translated script all half-width characters are padded with the '#' byte, the 48-character limit applies to them as well.
...
.area 4*32
	nop
	nop; 0x884C974: relocation-moved(0x884C954)
	nop; 0x884C978: relocation-moved(0x884C958)
	nop
	nop
	nop; 0x884C984: relocation-moved(0x884C964)
	nop
	nop
	nop
	nop
